# The below script was used to analyze the outputs of the model simulations from a nutrient uptake-oriented aspect.
...
library(mgcv);library(nlme);library(vegan);library(data.table)
...
library(dplyr);library(scales);library(stringr)
library(cowplot);library(lemon);library(tidyverse)
...
# (I) CONSTANT LOAD SIMULATIONS
# (1) Data processing
# Creating a common data frame for all simulation outputs (total_bm),
# containing daily values of total abundance, nutrient concentrations, compositional shift (dissimilarity),
# nutrient uptake, R*, net growth time and thermal optimum (T_opt).
...
# (2) GAMM fits for temporal changes in total biovolume (annual pattern of daily changes)
# The same code was used for all other variables presented this way.
# (nutrient concentrations, nutrient uptake, dissimilarity, mean growth rate and T_opt)
# (2.1) Model fitting
...
# use the same code for all other scenarios
...
# (5) Aggregating daily values into annual metrics for year 2
...
# (5.2) Annual nutrient uptake efficiency
...
# (6) Bray-Curtis dissimilarities between baseline and elevated temperature scenarios
# (6.1) Daily values
total_bm$present_dissim <- NA_real_
for(scen in 2:4) {
  for(lo in 1:9) {
...
# (6.2) Aggregating into annual means
...
# (7) Computing R* values for N and P
# (7.1) Daily biovolume-weighted values
...
# (8) Determining net growth time
# (8.1) Determining community-weighted growth rates first
total_bm$vol_mean_growth_rate <- NA_real_
# Defining function for growth rate calculation (vectorized version)
...
# (8.2) Determining net growth time
...
# (9) Computing weighted mean thermal optimum (T_opt)
# helper functions for T_opt calculation
monod_temp <- function(growth_par, nut1, nut2, temper) {
...
get_topt <- function(growth_par, nut1, nut2) {
  
  res <- optimize(
    f = monod_temp,
    interval = c(5, 45),
    maximum = TRUE,
    growth_par = growth_par,
    nut1 = nut1,
    nut2 = nut2
  )
...
# (9.1) Daily values 
total_bm$vol_mean_T_opt <- NA_real_
  ...
  for(lo in 1:9) {
    for(sp in 1:3) {
      for(com in 1:100) {
        ...
        growth_par_N <- data.frame(communities.random[[sp]]$N[[com]][,c("b1","b2","d0","d1","d2","K")])
        growth_par_P <- data.frame(communities.random[[sp]]$P[[com]][,c("b1","b2","d0","d1","d2","K")])
        growth_pars  <- data.frame(growth_par_N, growth_par_P)
        growth_pars  <- as.matrix(growth_pars)
        n_species    <- nrow(growth_pars)
        
        tempsim      <- t(as.matrix(sim_phyto_random_9_vol[[scen]][[lo]][[((sp-1)*100+com)]]))
        tempsim_total <- rowSums(sim_phyto_random_9_vol[[scen]][[lo]][[((sp-1)*100+com)]])
        vector_N     <- sim_phyto_random_9[[scen]][[lo]][[((sp-1)*100+com)]]$N[366:730]
        vector_P     <- sim_phyto_random_9[[scen]][[lo]][[((sp-1)*100+com)]]$P[366:730]
        n_days       <- length(vector_N)
        
        topt_matrix  <- matrix(NA, nrow = n_species, ncol = n_days)
        
        for(i in 1:n_species) {
          topt_matrix[i, ] <- mapply(
            get_topt,
            nut1 = vector_N,
            nut2 = vector_P,
            MoreArgs = list(growth_par = growth_pars[i, ])
          )
        }
        
        mean_topt <- colSums(topt_matrix * tempsim) / tempsim_total
        mean_topt[tempsim_total == 0] <- NA
        ...
        results_dt <- data.table(
          ...
          community = (sp-1)*100+com,
          day = 366:730,
          vol_mean_T_opt = mean_topt
        )
        
        total_bm[results_dt,
                 vol_mean_T_opt := i.vol_mean_T_opt,
                 on = .(num_scen, nutrient_load, community, day)]
...
# (9.2) Annual means
annual_new$mean_vol_weighted_T_opt <- NA_real_
for(scen in 1:4) {
  for(lo in 1:9) {
    for(com in 1:300) {
      temp <- mean(total_bm_dt$vol_mean_T_opt[total_bm_dt$num_scen == scen &
                                                total_bm_dt$nutrient_load == lo &
                                                total_bm_dt$community == com], na.rm = TRUE)
      annual_new$mean_vol_weighted_T_opt[annual_new$num_scen == scen &
                                           annual_new$nutrient_load == lo &
                                           annual_new$community == com] <- temp
      print(c(scen, lo, com))
    }
  }
}
...
annual$mean_vol_weighted_T_opt_dif <- NA_real_
...
for(lo in 1:9) {
  for(com in 1:300) {
    ...
    annual$mean_vol_weighted_T_opt_dif[annual$nutrient_load == lo &
                                         annual$community == com] <-
      annual$mean_vol_weighted_T_opt[annual$nutrient_load == lo &
                                       annual$community == com] - temp$mean_vol_weighted_T_opt
...
# Due to the large amount of simulated data, simulation outputs were saved in separate files
# for each nutrient load level x nutrient load fluctuation combination.
# These combinations were created using the following categories:
...
#########
# FIGURES
# Fig. 2-5: constant load simulations; Fig. 6-7: fluctuating load simulations
# FIG. 2 (same as numbering in the manuscript)
# Showing GAMM estimated annual patterns under different temperature scenarios
# at constant nutrient loads with different N:P ratios
load("/Local_path/Biovol_Predictions.RData")
# (a) Total biovolume
# Hydrological conditions
daily_dis     <- (250*10^6)/365 # m3/d, constant daily discharge over the year (250*10^6 m3/year)
scen_colors <- c("Baseline" = "dodgerblue",
                 "Present" = "dodgerblue",
                 "+1°C" = "gold",
                 "+2°C" = "darkorange1",
                 "+3°C" = "#7F0000")
scen_alpha <- c("Baseline" = 0.7,
                "Present" = 0.7,
                "+1°C" = 1,
                "+2°C" = 1,
                "+3°C" = 1)
...
for(lo in c(7,5,3)){
  ...
  plots[[enumerate]] <-
    ...
    scale_color_manual(values = scen_colors,
                       drop = FALSE,
                       breaks = c("Baseline", "+1°C", "+2°C", "+3°C"),
                       labels = c("Baseline", "+1°C", "+2°C", "+3°C")) +
    scale_fill_manual(values = scen_colors,
                      drop = FALSE,
                      breaks = c("Baseline", "+1°C", "+2°C", "+3°C"),
                      labels = c("Baseline", "+1°C", "+2°C", "+3°C")) +
    scale_alpha_manual(values = scen_alpha, guide = "none") +
    ylab(expression(
...
# (d) Nitrogen uptake
load("/Local_path/Nutrient_Uptake_Predictions.RData")
...
for(lo in c(7,5,3)){
  ...
  plots[[enumerate]] <-
    ...
    theme(panel.grid = element_blank(),
          ...
          rect = element_rect(color = "black"),
          plot.margin = margin(t = 4, r = 4, b = 4, l = 4),
          axis.title.y = if(show_y) element_text(size = 10, margin = margin(r = 22), lineheight = 0.7) else element_blank(),
          axis.text.y = if(show_y) element_text(size = 8) else element_blank(),
          axis.title.x = element_blank(),
          axis.text.x = element_blank()) +
    geom_line(aes(color = Scenario, alpha = Scenario), linewidth = 0.5) +
    geom_ribbon(aes(x = Day, ymin = ci_lower, ymax = ci_upper, fill = Scenario),
                alpha = 0.25,
                inherit.aes = FALSE) +
    scale_color_manual(values = scen_colors,
...
# (e) Bray_Curtis Dissimilarity
load("/Local_path/Dissimilarity_Predictions.RData")
for(lo in c(7,5,3)){
  ...
  plots[[enumerate]] <-
    ...
    ggplot(aes(x = Day, y = fit), data = dfs[[3]][[lo]]) +
    theme_bw() +
    theme(panel.grid = element_blank(),
          legend.position = "none", aspect.ratio = 0.8,
          rect = element_rect(color = "black"),
          plot.margin = margin(t = 4, r = 4, b = 4, l = 4),
          axis.title.y = if (show_y) element_text(size = 10, margin = margin(r = 24), lineheight = 0.7) else element_blank(),
          axis.text.y = if (show_y) element_text(size = 8) else element_blank(),
...
main_plot <- wrap_plots(plots, nrow = 5, ncol = 3) &
  theme(plot.margin = margin(3, 0, 0, -30))

plot_grid(main_plot, legend, ncol = 1, rel_heights = c(1, 0.065))

rm(list = ls())

# FIGURE 3
load("/Local_path/Sim_Output_Table.RData")
annual$Scenario[annual_new$Scenario == "Present"] <- "Baseline"
annual_long <- annual %>%
...
scenvar_colors <- c("Baseline_N" = "dodgerblue3",
                    "Baseline_P" = "dodgerblue3",
                    "+3°C_N" = "orangered3",
                    "+3°C_P" = "orangered3")

df$ScenVar <- factor(df$ScenVar,
                     levels = c("Baseline_N",
                                "Baseline_P",
                                "+3°C_N",
                                "+3°C_P"))

plot_efficiency <- ggplot(df, aes(x = NP_ratio, y = Efficiency, color = ScenVar)) +
  
  geom_point(aes(shape = ScenVar), size = 1.25) +
  geom_smooth(aes(fill = ScenVar,
                  linetype = ScenVar),
              linewidth = 0.4,
              se = TRUE, alpha = 0.2) +
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.background = element_rect(fill = "white", color = "black", linewidth = 0.175)) +
...
plots[[1]] <- annual %>%
  ...
  geom_smooth(span = 1, alpha = 0.2, linewidth = 0.4) +
  labs(color = "N/P ratio", fill = "N/P ratio",
       y = "Increase in <i>&eta;<sub> P</i></sub><br>compared to baseline scenario") +
  ...
  scale_fill_gradient2(low = "lawngreen", mid = "skyblue4", high = "navyblue",
                       midpoint = 45) +
  scale_y_continuous(breaks = seq(0.06, 0.14, by = 0.02),
                     labels = scales::label_number(accuracy = 0.01))

# (b) Change in T_opt vs. mean dissimilarity from baseline
plots[[2]] <- annual %>%
  filter(num_scen == 4 & sp_number == 50 & NP_ratio > 16) %>%
  ggplot(aes(x = mean_present_dissim, y = mean_vol_weighted_T_opt_dif,
             color = positive_net_growth_days_dif)) +
  theme_bw() +
  theme(panel.grid = element_blank(),
  ...
  geom_point(shape = 16, size = 1.25) +
  labs(
    color = "Change in <i>t</i><sub>g</sub><br>(days)",
    x = "Bray-Curtis dissimilarity<br>from baseline scenario",
    y = "Change in weighted <i>T</i><sub>opt</sub><br>compared to baseline scenario (\u00B0C)"
  ) +
  scale_color_gradient2(low = "lawngreen", mid = "skyblue4", high = "navyblue",
                        midpoint = 45) +
  scale_fill_gradient2(low = "lawngreen", mid = "skyblue4", high = "navyblue",
  ...
  geom_hline(yintercept = 0, linetype = "dashed",
             color = "red4", linewidth = 0.4, alpha = 0.5)

main_plot <- wrap_plots(plots, nrow = 2, ncol = 1) &
  theme(plot.margin = margin(2, 2, 3, 3))
...
plot_rstar  <-
  annual %>%
  filter(num_scen == 4 & sp_number == 50 & NP_ratio > 16) %>%
  ggplot(aes(x = rstar_P_limit_mean_dif,
             y = P_uptake_efficiency_limit_dif,
             color = present_dissim_P_limit_mean)) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        plot.margin = margin(t = 3, r = 5, b = 3, l = 3),
        axis.title.x = ggtext::element_markdown(
  ...
  scale_color_gradient2(low = "lawngreen", mid = "skyblue4", high = "navyblue",
                        midpoint = 0.5) +
  labs(x = "Change in <i>R*<sub>P</sub></i><br>compared to baseline scenario<br>(<i>\u03BC</i>mol L<sup>-1</sup>)",
       y = "Change in <i>&eta;<sub>P</i></sub><br>compared to baseline scenario",
       color = "Bray-Curtis dissimilarity<br>from baseline scenario")

print(plot_rstar)

# Fluctuating load simulations
# FIGURE 6
rm(list = ls())
...
annual_var$load_new_name <- annual_var$load
levels(annual_var$load_new_name) <- c("Low","Medium","High")
levels(annual_var$variability) <-c("Constant","30 days","20 days","10 days")

p1 <- annual_var[annual_var$sp_number == 50 &
                   annual_var$num_scen == 4,] %>%
  ggplot(aes(x = interaction(load_new_name, variability, sep = "\n"),
  ...
  theme(
    ...
    panel.border = element_rect(color = "black", linewidth = 0.35, fill = NA),
    plot.margin = margin(t = 0, r = 3, b = 6, l = 3),
    legend.position = "top",
    legend.text = element_text(size = 9),
    legend.title = element_text(size = 10, hjust = 0.5, margin = margin(b = 6)),
    legend.title.position = "top",
  ...
  geom_boxplot(outliers= FALSE, alpha = 0.75, linewidth = 0.3) +
  labs(y = "Change in <i>&eta;<sub>P</i></sub><br>compared to baseline scenario",
       fill = "Load level") +
  scale_fill_manual(values = c("Low" = "darkolivegreen1",
                               "Medium" = "darkolivegreen4",
                               "High" = "darkgreen")) +
  geom_hline(yintercept = 0, linetype = "dashed",
             color = "red4", linewidth = 0.25, alpha = 0.5) +
  geom_vline(xintercept = c(3.5,6.5,9.5),
             linetype = "dotted",
             color = "grey65", linewidth = 0.5) +
  annotate("text", x = c(2,5,8,11), y = 0.4, size = 3.5,
...
p2 <- annual_var[annual_var$sp_number == 50 &
  ...
  theme(
    panel.grid = element_blank(),
    panel.border = element_rect(color = "black", linewidth = 0.35, fill = NA),
    plot.margin = margin(t = 6, r = 3, b = 3, l = 3),
    legend.position = "none",
  ...
  labs(x = "Load level-variability combination",
       y = "Change in <i>&eta;<sub>N</i></sub><br>compared to baseline scenario") +
  scale_fill_manual(values = c("Low" = "darkolivegreen1",
                               "Medium" = "darkolivegreen4",
                               "High" = "darkgreen")) +
  geom_hline(yintercept = 0, linetype = "dashed",
...
ellipse_layer <- which(
  sapply(p$layers, function(x) inherits(x$stat, "StatEllipse"))
)

p$layers[[ellipse_layer]]$aes_params$linewidth  <- 0.15

# Remove zero reference lines
p$layers <- p$layers[
  !sapply(p$layers, function(x)
    inherits(x$geom, "GeomHline") ||
      inherits(x$geom, "GeomVline")
  )
]
...
# Re-scaling loadings to match the scale of scores
...
loadings$label_y[loadings$var %in% c("Dissimilarity",
  ...
  loadings$label_y[loadings$var %in% c("Dissimilarity",
                                       ...
                                       "R*\"*\"[N]",
                                       "R*\"*\"[P]",
                                       "italic(t[g])")] + c(0.4,0.3,-0.15,0.3,0.25,0.3)
...
loadings$label_x[loadings$var %in% c("Dissimilarity",
  ...
  loadings$label_x[loadings$var %in% c("Dissimilarity",
                                       ...
                                       "R*\"*\"[N]",
                                       "R*\"*\"[P]",
                                       "italic(t[g])")] + c(-1.15,0.2,0.4,-0.5,0.21,0.1)
...
p + geom_segment(data = loadings,
                 ...
                 arrow = arrow(angle = 20, length = unit(0.175, "cm"),
                               type = "closed"),
                 color = "black",
                 linewidth = 0.3) +
  ...
  labs(x = x_lab, y = y_lab,
       color = "Nutrient load", fill = "Nutrient load", shape = "Nutrient load") +
  theme(panel.grid = element_blank(),
        plot.margin = margin(t = 2, r = 2, b = 2, l = 2),
        ...
        legend.margin = margin(2,2,2,2),
        legend.title = element_text(size = 7, hjust = 0.5, margin = margin(b = 3)),
        legend.text = element_text(size = 7, margin = margin(l = 4)),
...
# FIGURE S1
# Increase in annual phosphorus uptake efficiency vs. changes in biovolume-weighted mean annual R* for P
load("/Local_path/Sim_Output_Table.RData")

annual %>%
  filter(num_scen == 4 & sp_number == 50 & NP_ratio > 16) %>%
  ggplot(aes(x = mean_vol_weighted_rstar_P_dif, y = P_uptake_efficiency_dif,
             color = NP_ratio, fill = NP_ratio, group = NP_ratio)) +
  theme_bw() +
  theme(panel.grid = element_blank(),
  ...
  labs(color = expression(atop("N/P load", "ratio")),
       fill = expression(atop("N/P load", "ratio")),
       y = expression(
         atop(
       ...
       x = expression(atop(paste("Change in weighted ",italic("R*"["P"])),
                           "compared to baseline scenario"))) +
  scale_color_gradient2(low = "lawngreen", mid = "skyblue4", high = "navyblue",
                        midpoint = 45) +
  scale_fill_gradient2(low = "lawngreen", mid = "skyblue4", high = "navyblue",
...
# FIGURE S2
# GAMM-estimated annual pattern of biovolume-weighted mean growth rate
rm(list = ls()[which(ls() != "sources")])
load("/Local_path/gamm_growth_predictions.RData")

daily_dis     <- (250*10^6)/365
scen_colors <- c("Baseline" = "dodgerblue",
                 "+1°C" = "gold",
                 "+2°C" = "darkorange1",
                 "+3°C" = "#7F0000")
scen_alpha <- c("Baseline" = 0.7,
                ...
                "+2°C" = 1,
                "+3°C" = 1)
max_growth <- max(df_growth[[3]][[3]]$fit, df_growth[[3]][[5]]$fit, df_growth[[3]][[7]]$fit)
min_growth <- min(df_growth[[3]][[3]]$fit, df_growth[[3]][[5]]$fit, df_growth[[3]][[7]]$fit)
...
for(lo in c(7,5,3)){
  ...
  df_growth[[3]][[lo]]$Scenario <- gsub("\u02DA", "\u00B0", df_growth[[3]][[lo]]$Scenario)
  df_growth[[3]][[lo]]$Scenario <- factor(df_growth[[3]][[lo]]$Scenario, levels = c("+3°C", "+2°C", "+1°C", "Baseline"))
  
  plots[[enumerate]] <-
    
    ggplot(aes(x = Day, y = fit), data = df_growth[[3]][[lo]]) +
    theme_bw() +
    theme(panel.grid = element_blank(),
          legend.position = "none",
          rect = element_rect(color = "black"),
          ...
          axis.title.y = if(show_y) element_text(size = 10, margin = margin(r = 3)) else element_blank(),
          axis.text.y = if(show_y) element_text(size = 7) else element_blank(),
          axis.title.x = element_blank(),
          axis.text.x = element_blank()) +
    geom_line(aes(color = Scenario, alpha = Scenario), linewidth = 0.25) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "grey35", linewidth = 0.25) +
    geom_ribbon(aes(x = Day, ymin = ci_lower, ymax = ci_upper, fill = Scenario),
                alpha = 0.25,
                inherit.aes = FALSE) +
    scale_color_manual(values = scen_colors) +
    ...
    ylab(expression(
      atop("Weighted mean growth rate",
           "(d⁻¹)")
    )) +
    ylim(min_growth, max_growth * 1.1) +
    guides(color = guide_legend(reverse=TRUE),
           fill = guide_legend(reverse=TRUE))
}

# GAMM-estimated annual pattern of biovolume-weighted mean T_opt
load("/Local_path/gamm_T_opt_predictions.RData")
...
max_T_opt <- max(df_T_opt[[3]][[3]]$fit, df_T_opt[[3]][[5]]$fit, df_T_opt[[3]][[7]]$fit)
min_T_opt <- min(df_T_opt[[3]][[3]]$fit, df_T_opt[[3]][[5]]$fit, df_T_opt[[3]][[7]]$fit)
...
for(lo in c(7,5,3)){
  ...
  df_T_opt[[3]][[lo]]$Scenario <- gsub("\u02DA", "\u00B0", df_T_opt[[3]][[lo]]$Scenario)
  
  plots[[enumerate]] <-
    
    ggplot(aes(x = Day, y = fit), data = df_T_opt[[3]][[lo]]) +
    theme_bw() +
    theme(panel.grid = element_blank(),
          legend.position = "none",
          rect = element_rect(color = "black"),
    ...
    scale_alpha_manual(values = scen_alpha, guide = "none") +
    ylab(expression(
      atop("Weighted mean" ~ italic(T[opt]),
           "(\u00B0C)")
    )) +
    ylim(min_T_opt*0.9, max_T_opt * 1.1) +
    guides(color = guide_legend(reverse=TRUE),
           fill = guide_legend(reverse=TRUE))
}
...
# FIGURE S3
# Changes in phosphorus uptake efficiency vs. changes in biovolume-weighted mean Topt
# in response to +3°C warming
load("/Local_path/Sim_Output_Table.RData")
plot_t_opt  <-
  annual %>%
  filter(num_scen == 4 & sp_number == 50 & NP_ratio > 16) %>%
  ggplot(aes(x = T_opt_P_limit_mean_dif,
             y = P_uptake_efficiency_limit_dif,
             color = present_dissim_P_limit_mean)) +
  theme_bw() +
  theme(panel.grid = element_blank(),
  ...
  scale_color_gradient2(low = "lawngreen", mid = "skyblue4", high = "navyblue",
                        midpoint = 0.5) +
  labs(x = "Change in <i>T<sub>opt</sub></i><br>compared to baseline scenario<br>(\u00B0C)",
       y = "Change in <i>&eta;<sub>P</i></sub><br>compared to baseline scenario",
       color = "Bray-Curtis dissimilarity<br>from baseline scenario")
...
# FIGURE S4
# Increase in annual phosphorus uptake efficiency vs. changes in annual net growth time
# under fluctuating nutrient load conditions
load("/Local_path/Varload_annual_values.RData")

annual_var$group <- interaction(annual_var$load, annual_var$variability, sep = " level - ")
group_cols <- c("low level - constant"  = "olivedrab3",
                "low level - 10 days"   = "steelblue1",
                "high level - constant" = "darkgreen",
                "high level - 10 days"  = "dodgerblue4")
...
df <- annual_var %>%
  filter(
    sp_number == 50,
    num_scen == 4,
    variability %in% c("constant", "10 days"),
    load %in% c("low", "high")
  )
...
# Ranges
#----------------------------
x_range <- range(df$positive_growth_days_dif, na.rm = TRUE)
y_range <- range(df$P_uptake_efficiency_dif, na.rm = TRUE)

x_lim <- c(x_range[1] * 1.15, x_range[2] * 1.10)
y_lim <- c(y_range[1] * 1.15, y_range[2] * 1.15)
  ...
  labs(
    x = expression(
      atop(
        ...
        "compared to baseline scenario (days)"
      )
    ),
    y = expression(
      atop(
...
# Legend panel
#----------------------------
legend_plot <- ggplot(df, aes(
  x = positive_growth_days_dif,
  y = P_uptake_efficiency_dif,
  color = group
)) +
  geom_point(shape = 1, size = 6, stroke = 2) +
  scale_color_manual(values = group_cols) +
  theme_void() +
  theme(
    legend.position = "right",
    legend.title = element_text(size = 33, hjust = 0.5),
...
legend_only <- patchwork::wrap_elements(
  get_legend(legend_plot)
)
#----------------------------
# Final layout
#----------------------------
left_side  <- xplot / p + plot_layout(heights = c(1.1,4))
right_side_content <- (wrap_elements(get_legend(legend_plot)) / wrap_elements(yplot)) + plot_layout(heights = c(1.1,4))

final_plot <- (left_side | right_side_content | plot_spacer()) + 
  plot_layout(widths = c(4, 1.15, 0.35))
